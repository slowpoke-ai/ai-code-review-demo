...
- **PR #8**（`UserProfile.tsx`）：前端组件，包含 XSS（innerHTML/dangerouslySetInnerHTML）、Open Redirect、postMessage 无 origin 校验、localStorage 存储敏感 token 等前端典型漏洞，CodeQL 触发 3 条 alert（1 High + 2 Medium）
...
| 步骤 | 工具 | 作用 |
|---|---|---|
| 类型检查 | `tsc --noEmit` | 只做类型校验不产出编译产物，发现类型错误立即报错 |
| Lint | `eslint --max-warnings 0` | 零警告容忍，有任何 ESLint warning 就 fail |
| 单元测试 | `vitest run --coverage` | 运行所有测试用例，同时生成 lcov 覆盖率报告 |
| 覆盖率上传 | `codecov/codecov-action` | 上传到 Codecov，PR 下自动评论覆盖率差异 |

---

### 🛡 security.yml — 安全扫描

**触发条件：** PR、push main，以及每周一凌晨 2 点定时全量扫描（持续检测存量风险 + 新公开 CVE）

**三个并行 Job：**

| Job | 工具 | 检测内容 |
|---|---|---|
| 依赖审计 | `npm audit --audit-level=high` | 扫描 package-lock.json 所有依赖的已知 CVE，high 以上报错 |
| CodeQL 静态分析 | `github/codeql-action` | 数据流分析（污点追踪），追踪从 source 到 sink 的完整路径，覆盖 XSS、SQL 注入、Open Redirect 等 100+ 种漏洞，结果上传 Security 面板并附带 Copilot Autofix |
| 密钥泄露扫描 | `gitleaks/gitleaks-action` | 扫描完整 git 历史（含已删除内容），检测 AWS Key、Stripe Secret、JWT、数据库密码等 100+ 种格式 |
...
- 自研 workflow 的唯一剩余价值是高度定制化的逻辑，但维护成本不低
...
1. **Dependabot** — 最低成本，加一个配置文件就搞定，防止依赖 CVE 被动发现
2. **`pr-hygiene.yml`** — PR 标题规范是 `release.yml` 自动发版的前提，先把格式卡住
3. **`release.yml`** — semantic-release 配置完成后，版本管理和 CHANGELOG 全自动
4. **`deploy.yml`（Vercel）** — 预览环境大幅降低 Review 成本，Reviewer 可直接点链接验证
...
- [CodeRabbit Trust Center](https://www.coderabbit.ai/trust-center) — 安全与隐私声明
- [Kudelski Security — CodeRabbit RCE 漏洞披露（已修复）](https://kudelskisecurity.com/research/how-we-exploited-coderabbit-from-a-simple-pr-to-rce-and-write-access-on-1m-repositories)
